...
2. 未指定时，简单 `2x2` 或 `2x4` 单业务且实际展示独立事实不超过 3 项、仅一个内容组、显式动作不超过 1 个，可使用对应融球；运行时允许且业务匹配时优先融球。单会议时间线使用暖色微渐变。列表、双分区、多组指标使用浅色微渐变，不为使用融球删掉用户必需内容。
...
1. **输出与协议**：是否只有一个 `genui` 代码块和可解析的极简协议 JSONL；是否没有 createSurface/updateComponents/updateDataModel/surfaceId/catalogId；root、组件字段和枚举是否正确；用户未指定配色时 root 是否只使用蓝/紫/暖微渐变或合法融球；融球是否满足第十二节的 2x2/2x4、单业务、最终信息项不超过 3、单内容组、显式动作不超过 1 和运行时条件，且 root 未同时写普通背景。
